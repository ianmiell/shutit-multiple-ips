...
class shutit_multiple_ips(ShutItModule):
	...
	def build(self, shutit):
		...
		shutit.build['module_name'] = 'shutit_multiple_ips_' + ''.join(random.choice(string.ascii_letters + string.digits) for _ in range(6))
...
/swapfile none            swap    sw              0       0" >> /etc/fstab''')
		...
		for machine in sorted(machines.keys()):
			...
			shutit_session.send('echo iface enp0s8 inet static >> /etc/network/interfaces')
			shutit_session.send('echo address 172.28.129.3/24 >> /etc/network/interfaces')
			shutit_session.send('ifdown enp0s8 && ifup enp0s8')
			shutit_session.send('ip addr')
			shutit_session.pause_point('hostname')
...
def module():
	return shutit_multiple_ips(
		'git.shutit_multiple_ips.shutit_multiple_ips', 269934500.0001,
		description='',
		maintainer='',
